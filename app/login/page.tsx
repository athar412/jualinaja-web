...
function LoginFormInner() {
  ...
  async function onSubmit(data: FormData) {
    ...
    if (result?.error) {
      if (result.error.includes("EMAIL_NOT_VERIFIED") || result.code === "EMAIL_NOT_VERIFIED") {
        setServerError("Akun Anda belum diverifikasi. Silakan periksa email Anda atau daftar ulang.");
      } else {
  ...
  return (
    <div className="w-full max-w-sm">
      <Link href="/" className="text-[15px] font-medium tracking-tight-luxury block mb-12 lg:hidden">
        jualinaja
      </Link>

      <h1 className="text-xl font-medium tracking-tight-luxury mb-1">Masuk</h1>
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-muted items-center justify-center p-12">
        <div className="max-w-xs">
          <Link href="/" className="text-[15px] font-medium tracking-tight-luxury block mb-12">
            jualinaja
          </Link>
          <h2 className="text-3xl font-medium tracking-tight-luxury leading-tight mb-4">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <Suspense fallback={<p>Memuat form...</p>}>
          <LoginFormInner />
        </Suspense>
      </div>
    </div>
